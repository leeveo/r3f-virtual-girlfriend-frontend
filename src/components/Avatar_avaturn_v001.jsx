...
export function Avatar(props) {
  ...
  useEffect(() => {
    // Alterner entre les expressions de manière aléatoire toutes les 3 secondes
    const timer = setInterval(switchExpression, 3000);
  ...
  useFrame((_, delta) => {
    ...
    Object.values(corresponding).forEach((morph) => {
      ...
      const boosted = ["viseme_aa", "viseme_O", "viseme_U", "viseme_E", "viseme_I"].includes(morph)
        ? target * 1.6 // Augmente l'ouverture pour ces visèmes spécifiques
        : target;

      lerpMorphTarget(morph, boosted, 0.2); // Interpolation fluide
    });
    ...
    const jawOpenTarget = ["viseme_aa", "viseme_O", "viseme_U", "viseme_E"].includes(corresponding[activeViseme])
      ? THREE.MathUtils.clamp(intensity * 1.5, 0, 1)
      : 0;

    const smoothedJawOpen = THREE.MathUtils.lerp(prevMorphs.jawOpen || 0, jawOpenTarget, 0.1); // Lissage progressif
    lerpMorphTarget("jawOpen", smoothedJawOpen, 0.05);
